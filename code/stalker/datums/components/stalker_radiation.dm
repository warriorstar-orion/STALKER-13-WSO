 ...
 */
/datum/component/stalker_rads_emitter
	...
	/// How much chance is passed to the radiation pulse (see: [proc/radiation_pulse])
	var/chance = 100
	/// How much minimum exposure time is passed to the radiation pulse (see: [proc/radiation_pulse])
	var/minimum_exposure_time = 0
	/// How much rads is passed to the radiation pulse (see: [proc/radiation_pulse])
	var/rads = DEFAULT_RADS_AMOUNT
...
/datum/component/stalker_rads_emitter/Destroy(force, silent)
	STOP_PROCESSING(SSfastprocess, src)
	return ..()

/datum/component/stalker_rads_emitter/RegisterWithParent()
	RegisterSignal(parent, COMSIG_ATOM_EXAMINE, PROC_REF(on_examine))

/datum/component/stalker_rads_emitter/UnregisterFromParent()
	UnregisterSignal(parent, COMSIG_ATOM_EXAMINE)

/datum/component/stalker_rads_emitter/InheritComponent(
...
/datum/component/stalker_rads_emitter/process(seconds_per_tick)
	if(!COOLDOWN_FINISHED(src, rad_pulse_cooldown))
		return

	COOLDOWN_START(src, rad_pulse_cooldown, cooldown_time)
	radiation_pulse(parent, range, threshold, chance, minimum_exposure_time, rads)

/datum/component/stalker_rads_emitter/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
