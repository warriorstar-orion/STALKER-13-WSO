...
/obj/structure/flora/bush/wildplant/attack_hand(mob/user)
	if(!ispath(produce))
		return ..()
...
/obj/structure/flora/bush/wildplant/regrow()
	. = ..()
	if(!QDELETED(src))
		has_plod = TRUE
		update_icon()

/obj/structure/flora/bush/wildplant/update_icon()
	. = ..()
	if(has_plod)
...
/obj/structure/flora/bush/wildplant/root
	name = "wild weeds"
	desc = "A useless weed which serves no purpose but to steal from other plants."
	icon_state = "wild_root1"
	anchored = 1
...
/obj/structure/flora/bush/wildplant/root/wild_potato
	name = "wild potato"
